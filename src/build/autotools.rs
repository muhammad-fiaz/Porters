...
impl BuildSystem for AutotoolsBuildSystem {
    ...
    fn configure(&self, _sources: &ProjectSources, deps: &[ResolvedDependency]) -> Result<()> {
        ...
        let output = cmd.output()
            .with_context(|| "Failed to run ./configure")?;
        
        if !output.status.success() {
            print_error(&format!("Configure failed:\n{}", String::from_utf8_lossy(&output.stderr)));
    ...
    fn build(&self, sources: &ProjectSources, deps: &[ResolvedDependency], args: &[String]) -> Result<()> {
        ...
        if !output.status.success() {
            print_error(&format!("Make failed:\n{}", String::from_utf8_lossy(&output.stderr)));
    ...
    fn run(&self, args: &[String]) -> Result<()> {
        print_warning("Autotools doesn't define a standard run target. Please run the installed executable.");
        print_info(&format!("Try: make install, then run the executable with: {}", args.join(" ")));
        Ok(())
    }
    ...
    fn test(&self, _sources: &ProjectSources, _deps: &[ResolvedDependency]) -> Result<()> {
        ...
        let output = cmd.output()
            .with_context(|| "Failed to run make check")?;
