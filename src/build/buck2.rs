...
impl BuildSystem for Buck2BuildSystem {
    ...
    fn run(&self, args: &[String]) -> Result<()> {
        ...
        let output = cmd.output().with_context(|| "Failed to run buck2 run")?;
    ...
    fn test(&self, _sources: &ProjectSources, _deps: &[ResolvedDependency]) -> Result<()> {
        ...
        let output = cmd.output().with_context(|| "Failed to run buck2 test")?;
    ...
    fn clean(&self) -> Result<()> {
        ...
        cmd.output().with_context(|| "Failed to run buck2 clean")?;
