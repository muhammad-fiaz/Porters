...
impl BuildSystem for MesonBuildSystem {
    ...
    fn configure(&self, _sources: &ProjectSources, _deps: &[ResolvedDependency]) -> Result<()> {
        ...
        if !output.status.success() {
            print_error(&format!("Meson configuration failed:\n{}", String::from_utf8_lossy(&output.stderr)));
    ...
    fn build(&self, sources: &ProjectSources, deps: &[ResolvedDependency], args: &[String]) -> Result<()> {
        ...
        let output = cmd.current_dir(&self.root)
            .output()
    ...
    fn run(&self, args: &[String]) -> Result<()> {
        ...
        let status = cmd.current_dir(&self.root)
            .status()
    ...
    fn clean(&self) -> Result<()> {
        ...
        if Path::new("build").exists() {
            std::fs::remove_dir_all("build")
                .with_context(|| "Failed to remove build directory")?;
        }
