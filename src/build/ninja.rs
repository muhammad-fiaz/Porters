...
impl BuildSystem for NinjaBuildSystem {
    ...
    fn build(&self, _sources: &ProjectSources, _deps: &[ResolvedDependency], args: &[String]) -> Result<()> {
        ...
        if !output.status.success() {
            print_error(&format!("Ninja build failed:\n{}", String::from_utf8_lossy(&output.stderr)));
    ...
    fn run(&self, args: &[String]) -> Result<()> {
        print_warning("Ninja doesn't define a standard run target. Please run the executable manually.");
    ...
    fn test(&self, _sources: &ProjectSources, _deps: &[ResolvedDependency]) -> Result<()> {
        ...
        let output = cmd.output()
            .with_context(|| "Failed to run ninja test")?;
    ...
    fn clean(&self) -> Result<()> {
        ...
        cmd.output()
            .with_context(|| "Failed to run ninja clean")?;
