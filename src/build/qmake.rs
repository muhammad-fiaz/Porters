...
impl BuildSystem for QMakeBuildSystem {
    ...
    fn build(
        ...
        let output = cmd.output().with_context(|| "Failed to build")?;
    ...
    fn run(&self, args: &[String]) -> Result<()> {
        ...
        print_info(&format!(
            "Run the executable from the build directory with: {}",
            args.join(" ")
        ));
        Ok(())
    }
    ...
    fn test(&self, _sources: &ProjectSources, _deps: &[ResolvedDependency]) -> Result<()> {
        ...
        let output = cmd.output().with_context(|| "Failed to run tests")?;
    ...
    fn clean(&self) -> Result<()> {
        ...
        cmd.output().with_context(|| "Failed to clean")?;
