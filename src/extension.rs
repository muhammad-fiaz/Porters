...
impl ExtensionManager {
    ...
    pub fn install_extension(&mut self, name: &str, source: ExtensionSource) -> Result<()> {
        match source {
            ExtensionSource::CratesIo => {
                ...
                println!("📦 Looking for extension '{}' in extensions directory...", name);
                ...
                if dest.exists() {
                    ...
                    println!("   To install: Clone the extension to ~/.porters/extensions/{}", name);
    ...
    pub fn execute_hook(&self, hook_name: &str, context: &HookContext) -> Result<()> {
        for extension in &self.loaded_extensions {
            ...
            if let Some(script) = script {
                println!("🔌 Running {} hook from '{}'...", hook_name, extension.manifest.name);
        ...
        for extension in &self.loaded_extensions {
            for cmd in &extension.manifest.commands {
                if cmd.name == command_name {
                    println!("🔌 Running command '{}' from '{}'...", command_name, extension.manifest.name);
    ...
    /// Execute script
    fn execute_script(&self, script: &str, extension_path: &Path, context: &HookContext) -> Result<()> {
    ...
    pub fn create_template(name: &str, path: &Path) -> Result<()> {
        ...
        println!("✅ Extension template created at {}", extension_dir.display());
        println!("\n📝 Next steps:");
        println!("   1. Edit extension.toml to configure your extension");
        println!("   2. Add hook scripts in the hooks/ directory");
        println!("   3. Test locally with: porters extension install --path {}", extension_dir.display());
