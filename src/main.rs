...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Add { package, dev, optional, git, branch, tag } => add_dependency(&package, dev, optional, git, branch, tag).await,
        Commands::Remove { package } => remove_dependency(&package).await,
        Commands::Build { all_platforms, linux, windows, macos, args } => {
        ...
        Commands::Install { package, git, branch, tag } => install_package(&package, git, branch, tag).await,
        Commands::Sync { dev, optional, no_cache } => sync_dependencies(dev, optional, !no_cache).await,
        ...
        Commands::Compile { all_platforms, linux, windows, macos, baremetal, target } => {
...
async fn init_project() -> Result<()> {
    ...
    if has_sources {
        print_success(&format!("Detected {} C/C++ source files", sources.source_files.len()));
    }
...
async fn create_project(name: &str, use_defaults: bool) -> Result<()> {
    ...
    let (language, author, email, repo, build_system, project_type, entry_point, license) = if use_defaults {
        print_info("Using default settings...");
        ("both".to_string(), None, None, None, "cmake".to_string(), "application".to_string(), None, Some("Apache-2.0".to_string()))
    } else {
        get_project_details()?
    };
    ...
    create_porters_config_enhanced(name, &author, &email, &repo, &build_system, &project_type, &entry_point, &license)?;
...
fn get_project_details() -> Result<(String, Option<String>, Option<String>, Option<String>, String, String, Option<String>, Option<String>)> {
    ...
    let project_type = if type_idx == 0 { "application" } else { "library" }.to_string();
    ...
    let entry_point = if project_type == "library" {
        ...
        if lib_name.trim().is_empty() { None } else { Some(lib_name) }
    } else {
        None
    };
    ...
    let author = if author.trim().is_empty() { None } else { Some(author) };
    ...
    let repo = if repo.trim().is_empty() { None } else { Some(repo) };
    ...
    Ok((language, author, email, repo, build_system, project_type, entry_point, license))
}
...
fn create_project_structure(language: &str) -> Result<()> {
    ...
    match language {
        "c" => {
            std::fs::write("src/main.c", r#"#include <stdio.h>
...
"#)?;
        ...
        "cpp" => {
            std::fs::write("src/main.cpp", r#"#include <iostream>
...
"#)?;
    ...
    // Create .gitignore
    std::fs::write(".gitignore", r#"# Build directories
build/
out/
target/
vendor/
...
"#)?;
    ...
    // Create README
    std::fs::write("README.md", r#"# Project
...
fn create_porters_config_enhanced(
    ...
    // Keywords
    config.push_str(&format!("keywords = [\"{}\", \"c\", \"cpp\"]\n", project_type));
...
fn create_porters_config(
    ...
    create_porters_config_enhanced(name, author, email, repo, build_system, "application", &None, &Some("Apache-2.0".to_string()))
}
...
async fn add_dependency(package: &str, dev: bool, optional: bool, git: Option<String>, _branch: Option<String>, _tag: Option<String>) -> Result<()> {
    ...
    let source = if let Some(git_url) = git {
        git_url
    } else if package.starts_with("http://") || package.starts_with("https://") || package.starts_with("git@") {
        package.to_string()
    } else {
        package.to_string()
    };
...
fn execute_build_script(script: &str, script_type: &str) -> Result<()> {
    ...
    if !status.success() {
        anyhow::bail!("{} script failed with exit code: {:?}", script_type, status.code());
    }
...
fn verify_dependency_checksums(resolved_deps: &[deps::ResolvedDependency]) -> Result<()> {
    ...
    for dep in resolved_deps {
        ...
        let Some(locked_dep) = lock.dependencies.get(&dep.name) else {
            print_info(&format!("⚠️  {} not in lockfile, skipping checksum verification", dep.name));
            continue;
        };
        
        // Skip if lockfile has no checksum stored
        let Some(expected_checksum) = &locked_dep.checksum else {
            print_info(&format!("⚠️  {} has no checksum in lockfile, skipping verification", dep.name));
            continue;
        };
        ...
        if !dep_path.exists() {
            anyhow::bail!("Dependency {} not found at {}", dep.name, dep_path.display());
        }
...
async fn build_project(
    ...
    let bin_cache_dir = config.cache.dir.clone()
    ...
    let sources = scan::scan_project(".")?;
    print_success(&format!("Found {} source files", sources.source_files.len()));
    ...
    if bin_cache.is_enabled() {
        ...
        for dep in &resolved_deps {
            ...
            if dep_path.exists() {
                ...
                let build_hash = hash::calculate_file_hash(
                    &std::env::current_dir()?.join("porters.toml")
                )?;
    ...
    if bin_cache.is_enabled() {
        ...
        if build_dir.exists() {
            ...
            bin_cache.store(project_name, project_version, &source_hash, &build_hash, &build_dir)?;
        }
    }
...
async fn generate_lockfile() -> Result<()> {
    ...
    for dep in &resolved_deps {
        ...
        let source = match &dep.source {
            deps::DependencySource::Git { url, rev } => {
                lockfile::DependencySource::Git {
                    url: url.clone(),
                    rev: rev.clone(),
                    branch: None,
                    tag: None,
            ...
            deps::DependencySource::Path { path } => {
                lockfile::DependencySource::Path {
                    path: path.clone(),
                }
            }
            deps::DependencySource::Registry { registry } => {
                lockfile::DependencySource::Registry {
...
async fn publish_package(token: Option<String>, dry_run: bool) -> Result<()> {
    ...
    let token = token.or_else(|| std::env::var("GITHUB_TOKEN").ok())
        .ok_or_else(|| anyhow::anyhow!("GitHub token required. Use --token or set GITHUB_TOKEN environment variable"))?;
...
async fn install_package(package: &str, git: Option<String>, _branch: Option<String>, _tag: Option<String>) -> Result<()> {
    ...
    let pkg_name = if source.contains("://") || source.contains("@") {
        source.split('/').last().unwrap_or(package).trim_end_matches(".git")
    } else {
        package
    };
    ...
    print_success(&format!("Installed {} globally at {}", pkg_name, install_path.display()));
    print_info(&format!("Global packages directory: {}", packages_dir.display()));
...
async fn execute_custom_command(args: Vec<String>) -> Result<()> {
    ...
    if let Some(custom_cmd) = config.commands.iter().find(|c| &c.name == command_name) {
        ...
        if !status.success() {
            anyhow::bail!("Custom command '{}' failed with exit code: {:?}", command_name, status.code());
        }
        
        print_success(&format!("Command '{}' completed successfully! ✓", command_name));
    } else {
...
async fn sync_dependencies(include_dev: bool, include_optional: bool, use_cache: bool) -> Result<()> {
    ...
    let cache_dir = config.cache.dir.clone().unwrap_or_else(|| {
        dirs::home_dir().unwrap().join(".porters").join("cache")
    });
    ...
    let bin_cache = bin_cache::BinaryCache::new(bin_cache_dir, cache_enabled && config.cache.binary_cache);
    bin_cache.init()?;
    ...
    if !config.extensions.is_empty() {
        print_info(&format!("Auto-installing {} extensions from config...", config.extensions.len()));
        let mut ext_manager = extension::ExtensionManager::new()?;
        ...
        for ext_name in &config.extensions {
            ...
            if extensions.iter().any(|e| &e.manifest.name == ext_name) {
                print_info(&format!("📦 Extension '{}' already installed, skipping", ext_name));
                continue;
            }
            ...
            match ext_manager.install_extension(ext_name, extension::ExtensionSource::CratesIo) {
                Ok(_) => print_success(&format!("Extension '{}' installed successfully! 🔌", ext_name)),
                Err(e) => print_warning(&format!("Failed to install extension '{}': {}", ext_name, e)),
            }
        }
    }
    ...
    print_info(&format!("Using project dependencies directory: {}", ports_dir.display()));
    ...
    for (name, dep, is_dev) in &to_install {
        ...
        match dep {
            config::Dependency::Detailed { git: Some(url), branch, tag, .. } => {
            ...
            config::Dependency::Detailed { path: Some(path), .. } => {
                print_info(&format!("{} is a path dependency at {}", name, path));
            }
            config::Dependency::Simple(spec) => {
                print_warning(&format!("{}: Simple version spec not yet supported ({})", name, spec));
            }
            _ => {
...
fn print_dependency(name: &str, dep: &config::Dependency, tree: bool, indent: usize) {
    ...
    match dep {
        ...
        config::Dependency::Detailed { git: Some(url), tag, branch, .. } => {
...
async fn clean_cache(force: bool) -> Result<()> {
    ...
    if let Ok(stats) = dep_cache.stats() {
        print_info(&format!("Dependency cache: {} items, {}", stats.count, stats.human_size()));
    }
    
    if force {
        if let Ok(stats) = bin_cache.stats() {
            print_info(&format!("Binary cache: {} items, {}", stats.count, stats.human_size()));
        }
    }
...
async fn update_deps(latest: bool) -> Result<()> {
    ...
    for (name, dep) in &mut config.dependencies {
        if let config::Dependency::Detailed { git: Some(_url), tag, branch, .. } = dep {
...
async fn compile_cross(
    ...
    let cross_config = cross_compile::CrossCompileConfig {
        targets: config.cross_compile.targets.iter().map(|(k, v)| {
            (k.clone(), cross_compile::TargetConfig {
    ...
    if all_platforms {
        ...
        if let Some(target_str) = target {
            print_warning(&format!("Custom target triple not yet fully supported: {}", target_str));
        }
    }
    ...
    print_info(&format!("Compiling for {} target(s) using {}", targets.len(), build_system_str));
    
    let build_dirs = compiler.compile_all(&targets, &build_system_str)?;
    
    print_success(&format!("Successfully compiled for {} targets! 🎉", build_dirs.len()));
...
async fn execute_single_file(file: &str, args: Vec<String>) -> Result<()> {
    ...
    let compiler = if let Some(ref cfg) = config {
        if is_cpp {
            cfg.run.cpp_compiler.clone().unwrap_or_else(|| detect_cpp_compiler())
        } else {
    ...
    let exec_status = exec_cmd.status()
    ...
    if Path::new(output_name).exists() {
        std::fs::remove_file(output_name)
            .context("Failed to remove temporary executable")?;
    }
    ...
    if exec_status.success() {
        ...
        print_error(&format!("Program exited with code: {}", exec_status.code().unwrap_or(-1)));
...
fn check_path_setup() {
    ...
    if let Ok(path_var) = env::var("PATH") {
        ...
        // Already in PATH
        if paths.iter().any(|p| p.trim() == cargo_bin_str || p.trim() == cargo_bin.to_str().unwrap_or("")) {
            return;
        }
    }
    ...
    if cfg!(windows) {
        ...
        println!("    \"Path\",");
        println!("    [Environment]::GetEnvironmentVariable(\"Path\", \"User\") + \";{}\",", cargo_bin_str);
        println!("    \"User\"");
        println!("  )");
        println!();
