...
async fn resolve_dependency(name: &str, dep: &Dependency, cache_dir: &Path) -> Result<ResolvedDependency> {
    match dep {
        Dependency::Simple(version) => {
            // For now, simple dependencies are not supported (future registry feature)
            print_warning(&format!("Registry dependencies not yet supported: {} = {}", name, version));
        ...
        Dependency::Detailed {
            ...
            if let Some(git_url) = git {
                resolve_git_dependency(name, git_url, branch.as_deref(), tag.as_deref(), rev.as_deref(), cache_dir).await
            } else if let Some(local_path) = path {
                resolve_path_dependency(name, local_path)
            } else if let Some(_ver) = version {
                print_warning(&format!("Registry dependencies not yet supported: {}", name));
                Err(anyhow::anyhow!("Registry dependencies not yet supported"))
            } else {
                Err(anyhow::anyhow!("Invalid dependency specification for {}", name))
            }
        }
    }
}
...
async fn resolve_git_dependency(
    ...
    let dep_dir = cache_dir.join("sources").join(format!("{}-{}", name, short_hash));
...
fn resolve_path_dependency(name: &str, path: &str) -> Result<ResolvedDependency> {
    ...
    let abs_path = path_buf.canonicalize()
