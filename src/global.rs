...
impl GlobalConfig {
    ...
    pub fn add_package(&mut self, name: String, version: String, source: String, install_path: PathBuf) -> Result<()> {
        ...
        if let Some(package) = self.packages.remove(name) {
            ...
            if package.install_path.exists() {
                std::fs::remove_dir_all(&package.install_path)
                    .with_context(|| format!("Failed to remove {}", package.install_path.display()))?;
            }
        }
