...
impl PortersConfig {
    ...
    pub fn load<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let config: PortersConfig = toml::from_str(&content)
            .with_context(|| "Failed to parse porters.toml")?;
    ...
    pub fn add_dependency(&mut self, package: &str, dev: bool, optional: bool) -> Result<()> {
        let dep = if package.starts_with("http://") || package.starts_with("https://") || package.starts_with("git@") {
            // Git dependency
            Dependency::Detailed {
                version: None,
        ...
        let pkg_name = if package.contains("://") {
            package.split('/').last().unwrap_or(package).trim_end_matches(".git")
        } else {
            package
        };
...
pub struct TargetConfig {
    ...
    #[serde(skip_serializing_if = "Option::is_none", rename = "cmake-toolchain-file")]
